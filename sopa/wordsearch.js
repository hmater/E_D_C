...
(function(){
  ...
  function WordSearch(wrapEl, settings) {
    ...
    //palabras:
  const palabras3_ = ['principios', 'basicos', 'derecho', 'trabajo', 'cuestion', 'social', 'dubio', 'operario', 'norma', 'favorable', 'condicion', 'ventajosa', 'principio', 'continuidad', 'primacia', 'realidad', 'buena', 'positiva', 'trato', 'digno', 'trabajador', 'certeza', 'acoso', 'sexual', 'laboral,', 'mobbing', 'empleador', 'trabajador', 'variandi', 'jornada', 'activa', 'trabajo', 'pasiva', 'bisemanal', 'excepcional', 'feriado', 'legal', 'colectivo', 'permiso', 'post', 'natal', 'paternal', 'matrimonio', 'sueldo', 'sobresueldo', 'comision', 'prestaciones', 'publicidad', 'pago', 'remuneracion', 'sueldo', 'termino', 'contrato', 'mutuo', 'acuerdo', 'renuncia', 'contrato', 'plazo', 'fijo', 'obra', 'faena', 'despido', 'vias', 'hecho', 'abandono', 'necesidades', 'empresa', 'nulidad', 'despido', 'carta', 'perdon', 'causal', 'todo', 'evento', 'recargo', 'indirecto', 'auto']
  const palabras3 = [
    'PRINCIPIOS', 'BASICOS', 'DERECHO', 'TRABAJO', 'CUESTION', 'SOCIAL', 'DUBIO', 'OPERARIO', 'NORMA', 'FAVORABLE', 'CONDICION', 'VENTAJOSA', 'PRINCIPIO', 'CONTINUIDAD', 'PRIMACIA', 'REALIDAD', 'BUENA', 'POSITIVA', 'TRATO', 'DIGNO', 'TRABAJADOR', 'CERTEZA', 'ACOSO', 'SEXUAL', 'LABORAL', 'MOBBING', 'EMPLEADOR', 'TRABAJADOR', 'VARIANDI', 'JORNADA', 'TRABAJO', 'PASIVA', 'BISEMANAL', 'EXCEPCIONAL', 'FERIADO', 'LEGAL', 'COLECTIVO', 'PERMISO', 'POST', 'NATAL', 'PATERNAL', 'MATRIMONIO', 'SUELDO', 'SOBRESUELDO', 'COMISION', 'PRESTACIONES', 'PUBLICIDAD', 'PAGO', 'REMUNERACION', 'SUELDO', 'TERMINO', 'CONTRATO', 'MUTUO', 'ACUERDO', 'RENUNCIA', 'CONTRATO', 'PLAZO', 'FIJO', 'OBRA', 'FAENA', 'DESPIDO', 'VIAS', 'HECHO', 'ABANDONO', 'NECESIDADES', 'EMPRESA', 'NULIDAD', 'DESPIDO', 'CARTA', 'PERDON', 'CAUSAL', 'TODO', 'EVENTO', 'RECARGO', 'INDIRECTO', 'AUTO'
];
    function getSomeWords(array, N) {
    ...
    var default_settings = {
      ...
      'gridSize': 15,
      'words': getSomeWords(palabras3, 5),
	  'wordsList' : [],
      'debug': false
    }
  ...
  WordSearch.prototype.gameOver = function() {
    ...
    if(localStorage.getItem('puzzle')===null){
      puzzle_data = {i: 0, s: [null,null,null,null]};
      console.log("n" + puzzle_data)
    }else{
      puzzle_data = JSON.parse(localStorage.getItem('puzzle'))
      console.log("e" + puzzle_data)
    }
    
    puzzle_data.s[puzzle_data.i] = score;
    puzzle_data.i++;
    localStorage.setItem('puzzle', JSON.stringify(puzzle_data));
    console.log("i "+ puzzle_data.i+" -  s"+puzzle_data.s);
    
    if(puzzle_data.i>=5){ //all games are completed
      var total_score = puzzle_data.s[0]+puzzle_data.s[1]+puzzle_data.s[2]+puzzle_data.s[3];
      showScore(total_score,20,1) 
      ...
      //show puntaje
      document.getElementsByClassName("ws-area")[0].remove()
      document.getElementsByClassName("ws-words")[0].remove()
      document.getElementsByClassName("wrap")[0].innerHTML+= "<h3>Puntaje: "+total_score+"</h3>"
...
function next(n=0){
  ...
  if(puzzle_data.i>=4){ //all games are completed
    var total_score = puzzle_data.s[0]+puzzle_data.s[1]+puzzle_data.s[2]+puzzle_data.s[3]+n;
    showScore(total_score,20,1) 
    console.log("total_Score: ",total_score);
